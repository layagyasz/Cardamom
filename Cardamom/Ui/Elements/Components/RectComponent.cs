...
namespace Cardamom.Ui.Elements.Components
{
    internal class RectComponent
    {
        ...
        public Vector2f Size => _vertices[3].Position - _vertices[0].Position;

        public bool IsPointWithinBounds(Vector2f point)
        {
            return point.X >= _vertices[0].Position.X
                && point.Y >= _vertices[0].Position.Y
                && point.X <= _vertices[3].Position.X
                && point.Y <= _vertices [3].Position.Y;
        }
        ...
        public void SetAttributes(ClassAttributes attributes)
        {
            ...
            Vector2f[] outer =
            {
                inner[0] + new Vector2f(-attributes.BorderWidth[0], -attributes.BorderWidth[3]),
                inner[1] + new Vector2f(-attributes.BorderWidth[0], attributes.BorderWidth[1]),
                inner[2] + new Vector2f(attributes.BorderWidth[2], attributes.BorderWidth[1]),
                inner[3] + new Vector2f(attributes.BorderWidth[2], -attributes.BorderWidth[3])
            };
            for (uint i = 0; i < 4; ++i)
            {
